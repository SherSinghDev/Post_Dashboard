...
const regex = /if \(user\.role == "Coordinator"\) \{[\s\S]*?(?=else \{)/;

const replacement = `if (user.type == "stockorder") {
...
// wait, the regex will find the FIRST `if (user.role == "Coordinator") {` which might be in `/patients`!
// I need to only replace it inside `/pendingpatients`!
console.log("WAIT");
